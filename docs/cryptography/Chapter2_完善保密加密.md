...
!!! abstract "证明"
    ...
    **必要性**：
    假设加密方案是完善保密加密，则根据贝叶斯定理有：
    $$
    \Pr[M = m] = \Pr[M = m \mid C = c] = \frac{\Pr[C = c \mid M = m] \cdot \Pr[M = m]}{\Pr[C = c]}
    $$
...
!!! abstract "证明"
    ...
    **充分性**：
    ...
    因为 $m_0$ 是任意选择的，所以对所有 $c \in \mathcal{C}$ 和 $m \in \mathcal{M}$，都有 $\Pr[C = c] = \Pr[C = c \mid M = m]$ 成立，故此加密方案是完善保密的，上述定义又被称为**完美不可区分性（Perfect indistinguishability）**
...
在讨论一次一密的安全性前，我们注意到每个 $k$ 和 $m$ 都满足：
$$
\text{Dec}_k(\text{Enc}_k(m)) = k \oplus (k \oplus m) = m
$$

所以一次一密是一个正确的加密方案，进一步可以得到：**一次一密是完善保密加密。**

!!! abstract "证明"
    选定明文空间 $\mathcal{M}$ 上的分布，选定任意一个 $m \in \mathcal{M}, c \in \mathcal{C}$，一次一密有：
    $$
    \Pr[C = c \mid M = m] = \Pr[M \oplus K = c \mid M = m] = \Pr[m \oplus K = c] = \Pr[K = m \oplus c] = \frac{1}{2^l}
    $$
...
!!! abstract "证明"
    ...
    由于假设 $|\mathcal{K}| < |\mathcal{M}|$，则 $\exists \, m'$ 使得 $m' \in \mathcal{M}$ 但 $m' \notin \mathcal{M}(c)$，则有：
    $$
    \Pr[M = m' \mid C = c] = 0 \neq \Pr[M = m']
    $$
...
* 设加密方案 $(\text{Gen}, \text{Enc}, \text{Dec})$ 的明文空间为 $\mathcal{M}$，且 $|\mathcal{K}| = |\mathcal{M}| = |\mathcal{C}|$，则当且仅当下列条件成立时，此方案是完善保密加密：
    ...
    2. $\forall \, m \in \mathcal{M}$, $\forall \, c \in \mathcal{C}$，只存在唯一的密钥 $k \in \mathcal{K}$ 使得 $\text{Enc}_k(m)$ 输出 $c$